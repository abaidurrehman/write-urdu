...
const toolbarHierarchySpec = fs.readFileSync(path.join(root, 'specs', 'WU-PLAT-004B-basic-writer-toolbar-hierarchy.md'), 'utf8');
...
assert.deepStrictEqual(Toolbar.OUTPUT_ACTIONS, ['pdf', 'word', 'png', 'svg', 'preview', 'print'], 'Output action registry changed unexpectedly');
assert.deepStrictEqual(Toolbar.DOWNLOAD_ACTIONS, ['pdf', 'word', 'png', 'svg', 'text'], 'Download disclosure must own every downloadable format');
assert.deepStrictEqual(Toolbar.PRIMARY_DOCUMENT_ACTIONS, ['copy', 'preview', 'share', 'print'], 'First-level document hierarchy changed unexpectedly');
...
assert.match(toolbarRuntime, /setAction\(svg, 'svg', 'SVG'/, 'Current SVG action is missing');
assert.match(toolbarRuntime, /setAction\(textExport, 'text', 'Text file'/, 'Current text export action is missing');
...
assert.match(toolbarRuntime, /function createDownloadMenu/, 'Named Download disclosure is missing');
assert.match(toolbarRuntime, /data-wu-basic-download-toggle/, 'Download trigger contract is missing');
assert.match(toolbarRuntime, /data-wu-basic-download-panel/, 'Download format panel contract is missing');
assert.match(toolbarRuntime, /data-wu-basic-mobile-document-actions/, 'Mobile document overflow destination is missing');
assert.match(toolbarRuntime, /if \(compact\) destination\.appendChild\(print\)/, 'Print must move into More on compact screens');
assert.match(toolbarRuntime, /primary\.insertBefore\(print, more\)/, 'Print must return to the direct document row on desktop');
assert.match(toolbarRuntime, /primaryGroup\.appendChild\(copy\)[\s\S]*primaryGroup\.appendChild\(preview\)[\s\S]*primaryGroup\.appendChild\(download\)[\s\S]*primaryGroup\.appendChild\(share\)[\s\S]*primaryGroup\.appendChild\(print\)[\s\S]*primaryGroup\.appendChild\(more\)/, 'Desktop document hierarchy must be Copy, Preview, Download, Share, Print, More');
...
assert.match(toolbarCss, /wu-basic-command--copy/, 'Copy styling is missing');
assert.match(toolbarCss, /wu-basic-command--document/, 'First-level document-action styling is missing');
assert.match(toolbarCss, /wu-basic-command--download/, 'Download trigger styling is missing');
assert.match(toolbarCss, /wu-basic-command-download-panel/, 'Compact Download popover styling is missing');
assert.match(toolbarCss, /wu-basic-command-more-panel/, 'Compact More popover styling is missing');
assert.match(toolbarCss, /wu-basic-command--clear/, 'Destructive Clear styling is missing');
assert.match(toolbarCss, /\[data-wu-basic-mobile-document-actions\]\[hidden\]/, 'Mobile document overflow must guarantee real hiding');
...
assert.match(serviceWorker, /write-urdu-shell-v49/, 'PWA shell generation must remain on the current coordinated cache generation');
...
assert.match(toolbarSpec, /moving every export under `More` reduced export use/i, 'WU-PLAT-004 must retain the export-discovery evidence');
...
assert.match(toolbarHierarchySpec, /Copy → Preview → Download ▾ → Share → Print → More/, 'WU-PLAT-004B must govern the direct desktop hierarchy');
assert.match(toolbarHierarchySpec, /PDF;[\s\S]*Word;[\s\S]*PNG;[\s\S]*SVG;[\s\S]*Text file/, 'WU-PLAT-004B must keep all formats in Download');
assert.match(toolbarHierarchySpec, /move \*\*Print\*\* into `More`/, 'WU-PLAT-004B must define compact Print behavior');
assert.match(toolbarHierarchySpec, /Download remains visible but disabled/, 'WU-PLAT-004B must preserve E0 export discovery without a format wall');
