...
const appSource = read('js', 'writing-templates.js');
const app = require(path.join(root, 'js', 'writing-templates.js'));
...
assert.ok(page.indexOf('workspace-handoff.js') < page.indexOf('writing-templates.js'), 'Handoff runtime must load before writing templates');
...
assert.doesNotMatch(appSource, /env\.AI|Workers AI|indictrans|openai|anthropic/i, 'Writing templates must remain AI-free');
