...
assert.match(toolbar, /moving every export under `More` reduced export use/i, 'Basic Writer spec must record the latest export-discovery evidence');
...
assert.match(toolbar, /\*\*PDF\*\* — visible but disabled at E0; enabled E1\+/, 'PDF must stay discoverable without acting on empty content');
assert.match(toolbar, /\*\*Word\*\* — visible but disabled at E0; enabled E1\+/, 'Word must stay discoverable without acting on empty content');
assert.match(toolbar, /\*\*PNG\*\* — visible but disabled at E0; enabled E1\+/, 'PNG must stay discoverable without acting on empty content');
