...
- Confirm the canonical host is `https://write-urdu.com` and that HTTP, the apex alternate host, `.html` routes, and trailing-slash variants redirect directly to it.
...
- In Cloudflare, configure an HTTPS and canonical-host redirect with Redirect Rules or Bulk Redirects from HTTP and the non-canonical apex host to `https://write-urdu.com`. Keep it as one direct hop and test query strings and deep links.
