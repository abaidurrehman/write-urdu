...
- Confirm the canonical host is `https://www.write-urdu.com` and that HTTP, the apex alternate host, `.html` routes, and trailing-slash variants redirect directly to it.
...
- Verify that Googlebot, OAI-SearchBot, PerplexityBot, Bingbot and the explicitly listed AI crawlers are not blocked by CDN or firewall rules. Keep the `GPTBot` and `Google-Extended` policy decisions documented in `robots.txt`.
- In Cloudflare, configure an HTTPS and canonical-host redirect with Redirect Rules or Bulk Redirects from HTTP and the non-canonical apex host to `https://www.write-urdu.com`. Keep it as one direct hop and test query strings and deep links.
