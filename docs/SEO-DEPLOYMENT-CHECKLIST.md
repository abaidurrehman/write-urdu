...
1. Confirm `SITE_ORIGIN` remains `https://write-urdu.com` and that HTML canonicals, `og:url`, structured-data URLs and sitemap entries use that origin.
...
7. Keep or add a Google Search Console **Domain property** for `write-urdu.com`, submit only `https://write-urdu.com/sitemap.xml`, and inspect canonical `www` URLs for home, editor, keyboard, Roman Urdu guide, Documentation and About.
