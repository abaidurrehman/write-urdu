...
- **Edit step (`edit_<id>`)**: two-column layout (stacks to one column under `~700px`, matching the responsive rule every other page in this repo uses). Left/top: date, wording tone selector, "Edit wording" override control (`applyWordingOverride`, `isWordingStale` — unchanged from Slice 1); venue selection is not implemented in this slice (only a `venueId` schema field exists, with no picker UI in any slice so far). Right/bottom: **live preview pane** — the extracted per-event render helper (§3) re-runs on every relevant input's `oninput`/`onchange`, debounced 200ms so typing doesn't re-render on every keystroke.
- **Review/export step**: unchanged in purpose from today's step 6 — one rendered card per event (via the same extracted helper) with a "Download image" button (html2canvas). The staleness notice for a `wordingOverride` that no longer matches its source fields appears only on the edit step (as in Slice 1); the review/export step has no staleness notice of its own.
