...
- `suggestBackgroundCategory(project, event)` — pure function, no side effects. **Updated post-spec:** PR #231 shipped the first event-specific SVG art pack (`assets/wedding-invitations/riwaayat/manifest.json`, 6 variants each declaring their own `eventTypes`), closing the gap this section originally described (only 3 generic wedding-tagged backgrounds, none event-specific). `suggestBackgroundCategory` now cross-references the event's type directly against the manifest's own `eventTypes` tags — a more precise match than the originally planned `card-background-registry.js` `category`/`goodFor` cross-reference, and it deliberately does **not** read from `card-background-registry.js` at all, since that registry is Card Studio's own and the art pack's doc explicitly stays out of it until a dedicated wedding renderer/theme adapter exists. Returns a sorted list of matching variant IDs (may be empty for event types the pack doesn't cover yet, e.g. `rukhsati`/`engagement`/`custom` — empty is valid, never guessed).
