...
**Amendment 1 (made while writing the implementation plan, before any code):** the Slice 0 render adapter (`js/wedding-invitation-render-adapter.js`) builds a **Card Studio** canvas project seed, and Card Studio's actual canvas renderer resolves `background.imageAssetId` only through `js/card-background-registry.js`. Riwaayat's SVGs are not registered there — deliberately, per the Riwaayat art pack's own doc, which says it stays out of that registry until a dedicated wedding renderer/theme adapter exists. That adapter doesn't exist yet, so this slice cannot render a chosen Riwaayat background through Card Studio's canvas pipeline as originally scoped below. Resolution (owner's call): step 6 renders as plain DOM/CSS (the Riwaayat SVG as a CSS background image, wording text absolutely-positioned per the manifest's `safeArea`), exported via html2canvas directly against that DOM node. `wedding-invitation-render-adapter.js` and Card Studio are **not used anywhere in this slice** — every place below that mentions them is superseded by this amendment; see the corrected §1/§3.

**Amendment 2 (same pass):** `js/wedding-template-selector.js` only obtains the Riwaayat manifest via Node's `require()` for its Node fallback path; in a browser, nothing currently sets `window.WriteUrduRiwaayatManifest`, so the module would throw `Riwaayat design manifest unavailable` on page load. This slice adds `js/riwaayat-manifest.js` — a small UMD module that is the sole `require()` point for the manifest JSON in Node and sets `window.WriteUrduRiwaayatManifest` in the browser from an inlined copy of the same data — plus a contract test asserting the inlined copy stays byte-identical to `assets/wedding-invitations/riwaayat/manifest.json` (drift fails the test immediately, same discipline as this repo's other generated/source consistency contracts). `wedding-template-selector.js`'s Node fallback is updated to `require('./riwaayat-manifest.js')` instead of reaching into the JSON directly, so there is exactly one place that touches the raw JSON file.
...
3. `js/urdu-wedding-invitation-maker.js` — DOM wiring only (no new domain logic): reads `window.WriteUrduWeddingCore`, `window.WriteUrduWeddingWording`, `window.WriteUrduWeddingTemplateSelector`, `window.WriteUrduWeddingProjectStorage` (no render adapter, no Card Studio — see amendment above), renders the current step, handles Back/Next, calls `saveDraft`/`loadDraft`, and triggers image export via html2canvas directly against the step-6 preview DOM node.
...
5. `resolveEventBackground` and `getBackgroundVariant`, added to `js/wedding-template-selector.js` — see §2.
6. `js/riwaayat-manifest.js` — browser-visible copy of the Riwaayat manifest plus its sync-contract test (Amendment 2).
7. Route registration through the four canonical mechanisms (registry CSV, `seo.config.js`, `sitemap.xml`, `_redirects`).
8. A Playwright spec proving the real-browser composer flow (first for this epic's UI layer).
...
- Scripts, all `defer`, load order: `/seo.config.js`, `/js/seo.js`, `/site-header.js`, then the domain modules in dependency order — `/js/wedding-wording-registry.js`, `/js/riwaayat-manifest.js` (sets `window.WriteUrduRiwaayatManifest`, see Amendment 2), `/js/wedding-project-core.js`, `/js/wedding-template-selector.js`, `/js/wedding-project-storage.js`, then html2canvas (CDN, matching Card Studio/invoice-generator's existing loading pattern) — finally `/js/urdu-wedding-invitation-maker.js`, which guards `if (!core || !wording || !templateSelector || !storage) return;` before wiring anything, matching `bill-generator.js`'s existing guard convention. `card-background-registry.js` and `wedding-invitation-render-adapter.js` are not loaded on this page (see Amendment 1 — no Card Studio involvement).
...
- `selectedBackgroundId` — set only when the user explicitly picks a Riwaayat variant (or any future background) in step 5. Resolution order the UI uses to decide what to actually render: explicit `selectedBackgroundId` → first match from `suggestBackgroundCategory(project, event)` → `null` (meaning "no art variant available; the UI applies its own neutral CSS fallback look, per Amendment 1 — never a fabricated visual"). This resolution logic is a new small pure function, `resolveEventBackground(project, event)`, added to `wedding-template-selector.js` (it already owns "suggest, never auto-decide" concerns) — it never mutates the project, purely resolves what step 5/6 should display, and returns a variant id or `null`, never invents one. A companion accessor, `getBackgroundVariant(variantId)`, returns the full Riwaayat manifest entry (`src`, `textColor`, `overlayColor`, `overlayOpacity`, `safeArea`) for a given id so the DOM-wiring script never reaches into the manifest directly. `evaluateComposerSteps`'s existing `designDefault` field (`classic-nastaliq`/`portrait`) is untouched by this slice — it remains valid step-5-completion metadata from the logic slice, simply unused for rendering now that rendering is DOM/CSS-based (Amendment 1).
...
- **Step 5 (design)**: for each event, shows `suggestBackgroundCategory(project, event)`'s matches as clickable thumbnails (the Riwaayat SVG for that event type, rendered via `getBackgroundVariant`'s `src`). Clicking one sets `event.selectedBackgroundId`. If suggestions are empty (rukhsati/engagement/custom — not yet covered by Riwaayat), the step shows a plain neutral preview swatch (the same CSS fallback step 6 uses) with a note that no matching design exists yet — never a fabricated suggestion.
- **Step 6 (preview/export)**: for each event, renders a plain DOM preview element — `resolveEventBackground(project, event)` picks the variant id (or `null`); if non-null, `getBackgroundVariant` supplies the SVG `src` as a CSS `background-image` plus its `textColor`/`overlayColor`/`overlayOpacity`/`safeArea` to size and color the text box; if `null`, a single neutral fallback CSS class (ivory background, thin gold border, dark neutral text) is used instead. The rendered wording (via `renderWording`, or the event's `wordingOverride.text` when present) is placed in that text box. A "Download image" button per event runs html2canvas directly against that preview DOM node, producing one PNG per event. No PDF, no bundling, no guest links, no publish/share action, no Card Studio involvement (Amendment 1).
...
- `tests/wedding-template-selector.test.js` (extended): `resolveEventBackground` fixtures covering all three resolution-order cases (explicit selection wins; falls back to first suggestion; returns `null` when suggestions are empty, never a fabricated id), plus `getBackgroundVariant` fixtures (returns the full manifest entry for a known id, returns `null` for an unknown id).
- New `tests/riwaayat-manifest-sync-contract.test.js`: asserts `js/riwaayat-manifest.js`'s embedded data is deep-equal to `assets/wedding-invitations/riwaayat/manifest.json` (Amendment 2 — catches drift immediately).
