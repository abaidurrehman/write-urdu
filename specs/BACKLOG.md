...
**Not yet started, correctly so:** `WU-COMMUNITY-001` (F) and `WU-I18N-001D` have zero code and remain Planned/gated on their stated evidence. **Update 2026-08-29:** `WU-COMMUNITY-001A` (private submission schema + owner-only API), `WU-COMMUNITY-001B` (editor prompt/manual publish UX), `WU-COMMUNITY-001C` (Product OS moderation queue + approve/reject/unpublish, `functions/lib/community-moderation.mjs`, `os/community-writing.html`), `WU-COMMUNITY-001D` (public SSR hub/detail/category reader, dynamic community sitemap, report flow, `functions/lib/community-publications.mjs`) and `WU-COMMUNITY-001E` (`/my-publications` writer dashboard, revisions, self-withdrawal, `functions/lib/community-my-publications.mjs`, `migrations/0013_community_publication_withdrawal.sql`) now have shipped, test-covered code — see `specs/WU-COMMUNITY-001A-submission-data-api.md` through `specs/WU-COMMUNITY-001E-my-publications-revisions-withdrawal.md`. The feature now has a working end-to-end path (submit → moderate → read → revise/withdraw) but stays dark by default behind `COMMUNITY_PUBLIC_ENABLED` (no operator has flipped it) and is not yet linked from the main sitemap/robots registry (Slice F's job) or promotable per the parent spec's §25 launch gate — so it still stays gated from Tier 4 sequencing. Taxonomy/guidelines/launch closure (Slice F) remains zero code, plus the account-deletion retention contract that Slice E's own spec (§17) flags as a launch dependency, not yet resolved.  
