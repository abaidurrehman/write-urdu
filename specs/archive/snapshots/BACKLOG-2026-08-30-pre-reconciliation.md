...
**Not yet started, correctly so:** `WU-I18N-001D` has zero code and remains Planned/gated on its stated evidence. **Update 2026-08-29:** `WU-COMMUNITY-001A` (private submission schema + owner-only API), `WU-COMMUNITY-001B` (editor prompt/manual publish UX), `WU-COMMUNITY-001C` (Product OS moderation queue + approve/reject/unpublish, `functions/lib/community-moderation.mjs`, `os/community-writing.html`), `WU-COMMUNITY-001D` (public SSR hub/detail/category reader, dynamic community sitemap, report flow, `functions/lib/community-publications.mjs`), `WU-COMMUNITY-001E` (`/my-publications` writer dashboard, revisions, self-withdrawal, `functions/lib/community-my-publications.mjs`, `migrations/0013_community_publication_withdrawal.sql`) and `WU-COMMUNITY-001F` (category indexing threshold, `/community-guidelines`, privacy/terms reconciliation, ad classification, telemetry allowlist + reading-pulse rollup, OS operational pulse endpoint, robots/sitemap discovery of the dynamic community sitemap) now have shipped, test-covered code — see `specs/WU-COMMUNITY-001A-submission-data-api.md` through `specs/WU-COMMUNITY-001F-taxonomy-guidelines-launch-closure.md`. The feature is live in production behind `COMMUNITY_SUBMISSIONS_ENABLED=true` with a working end-to-end path (submit → moderate → read → revise/withdraw). `COMMUNITY_PUBLIC_ENABLED` stays off by deliberate product choice (staged rollout, spec §14 Stage 3) — `/urdu-writers` itself is still not in site nav/robots discovery, only `/community-guidelines` is. **Update 2026-08-29 (2):** closed the OS pulse view/CTA telemetry gap — `community_publication_viewed`/`community_write_cta_clicked` were already sent by `js/community-writers.js` and already allowlisted, but `functions/api/events.js` had no rollup branch for them, so they were accepted and silently discarded; `migrations/0014_community_telemetry_rollups.sql` adds bounded `community_views`/`community_cta_clicks` columns and the OS pulse now reports 7-day reading views/CTA clicks alongside queue/reports/approval-rate. Confirmed no account-deletion feature exists anywhere in the codebase yet, so Slice E's own §17 fallback ("document as launch dependency if account deletion isn't shipped") is the correct, already-satisfied state — nothing to build there until account deletion itself ships. Before broad promotion per the parent spec's §25 launch gate, still open and correctly not attempted here: legal review of the guidelines/terms wording, the product decision on when to flip `COMMUNITY_PUBLIC_ENABLED`/add nav discovery, the ad-density call for eventually monetizing `/urdu-writers`, and production toggling of the rollback flags — all human/product decisions, not code.  
