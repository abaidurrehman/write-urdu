...
| `WU-COMMUNITY-001` | Moderated Urdu Writing Publishing / Urdu Writers | `/urdu-writers`, `/urdu-writers/:slug`, `/my-publications`, Product OS moderation | Planned — Slices A (private submission schema + owner-only API), B (editor prompt/manual publish UX), C (Product OS moderation queue + approve/reject/unpublish), D (public SSR hub/detail/category, dynamic community sitemap, report flow) and E (`/my-publications` dashboard, revisions, writer withdrawal) shipped and test-covered; public surface stays dark by default behind `COMMUNITY_PUBLIC_ENABLED` (writer-facing submission/dashboard surface behind the separate `COMMUNITY_SUBMISSIONS_ENABLED`), not linked from the main sitemap/robots registry yet; taxonomy/guidelines launch closure (Slice F) remains zero code |
